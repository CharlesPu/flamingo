...
func (b *batchProcessorWithKey) notify() {
	...
	for obj := range b.buffer {
		if obj == nil {
			plog.Warnf("[batchProc] buffer receive sig to quit and force flush(keyN %d)", len(batchWithKey))
			flushAll()
			return
		}
		switch x := obj.(type) {
		case *timerSig:
			plog.Debugf("[batchProc] timeout and force flush(keyN %d)", len(batchWithKey))
			flushAll()
		...
		case *withKey:
			...
			if b.options.calculator != nil {
				...
				if batch.itemsWeight >= b.options.weightMax {
					plog.Debugf("[batchProc] over weight and flush(size %d, weight %d)", len(batch.data), batch.itemsWeight)
					flushOnce(x.key, batch)
					continue
				}
			}
			if len(batch.data) >= b.options.batchThreshold {
				plog.Debugf("[batchProc] size out and flush(size %d, weight %d)", len(batch.data), batch.itemsWeight)
				flushOnce(x.key, batch)
			}
		default:
			plog.Errorf("[batchProc] unknown obj type: %T", obj)
		}
	}
}
...
func (b *batchProcessorWithKey) cleanKey(m map[string]*withWeight) map[string]*withWeight {
	for k, v := range m {
		if len(v.data) == 0 {
			plog.Debugf("[batchProc] clean key: %s", k)
			delete(m, k)
